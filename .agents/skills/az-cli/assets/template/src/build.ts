...
const buildCJS = async () => {
	...
	if (!result.success) throw new Error('CJS build failed: ' + result.logs.map((l) => l.message).join(', '));
...
const buildESM = async () => {
	...
	if (!result.success) throw new Error('ESM build failed: ' + result.logs.map((l) => l.message).join(', '));
...
const buildCLI = async () => {
	...
	if (!result.success) throw new Error('CLI build failed: ' + result.logs.map((l) => l.message).join(', '));
